...
function isPublicContent(file) {
    return !/\.html?$/i.test(file) || !fs.statSync(file).isFile() || !pageProblem(fs.readFileSync(file, 'utf8'));
}
...
function publicEntries(root) {
    return fs.readdirSync(root, { withFileTypes: true }).filter(entry => {
        ...
        return files.has(entry.name) || (
            /\.html?$/i.test(entry.name) && !/^(test_|temp\.)/i.test(entry.name) && isPublicContent(path.join(root, entry.name))
        );
    }).map(entry => entry.name);
}

module.exports = { publicEntries, isPublicContent };
